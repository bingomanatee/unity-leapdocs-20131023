...
public class FS_ShadowManagerMesh : MonoBehaviour {
	...
	void _removeDestroyedShadows(){
		List<FS_ShadowSimple> newShadowsList = new List<FS_ShadowSimple>();
		for (int i = 0; i < shadows.Count; i++){
			if (shadows[i] != null){
				newShadowsList.Add(shadows[i]);	
			}
		}
		shadows = newShadowsList;
	}
	
	void _CreateGeometry(){		
		//check if any shadows have been destroyed
		for (int i = 0; i < shadows.Count; i++){
			if (shadows[i] == null){;
