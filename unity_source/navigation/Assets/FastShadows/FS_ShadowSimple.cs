...
public class FS_ShadowSimple : MonoBehaviour {
	...
	public void CalculateShadowGeometry(){
		...
		if (useLightSource){
			Vector3 ls = trans.position - lightSource.transform.position;
		...
		if (shadowCaster == null || girth != _girth){
			...
			if (Mathf.Abs(Vector3.Dot(trans.forward, lightDirection)) < .9f){
				proj = trans.forward - Vector3.Dot(trans.forward, lightDirection) * lightDirection; 
			} else {
				proj = trans.up - Vector3.Dot(trans.up, lightDirection) * lightDirection;
			}
		...
		if (maxProjectionDistance > 0f && Physics.Raycast(r, out rh, maxProjectionDistance, layerMask)){
			...
			t.rotation = Quaternion.LookRotation(proj, -lightDirection);			
